...
async def get_status(
    ...
    s3_client = S3Client()
    object_keys = s3_client.list_objects(prefix)  # Теперь ascending, oldest first
    total_files = len(object_keys)
    completed_files = 0
    if journal["last_completed_file"]:
        try:
            completed_idx = object_keys.index(journal["last_completed_file"])
            completed_files = completed_idx + 1  # Первые (старые) завершены
        except ValueError:
            pass
    current = (
